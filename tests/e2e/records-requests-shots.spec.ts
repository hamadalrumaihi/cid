 ...
 *  the judicial queue — P4-01, migration 20261025120000_legal_reroute, means no ISSUED
 *  warrant is reachable without the judge fixture; the D3
 *  execution/seized-items shot is skipped until it exists — issue #299, see
 *  tests/rls/v163.test.ts) and add, as the fixture users through
...
test.describe('Records & Requests — Phase-0 screenshot verification', () => {
  ...
  test.beforeAll(async () => {
    ...
    // D3 — needs an ISSUED warrant, which the judicial pipeline (P4-01,
    // 20261025120000_legal_reroute) makes unreachable for CID fixtures: judicialQueueWarrant
    // stops at submitted_to_judge. The seized-item add and the D3 shot are
    // skipped until the judge fixture is provisioned (issue #299,
    // tests/rls/v163.test.ts).
  ...
  test('D3 · warrant execution controls + seized-items panel', async ({ page }) => {
    // Requires an ISSUED warrant. Under the judicial pipeline (P4-01,
    // 20261025120000_legal_reroute) issuance needs a judge's approval, which the CID
    // fixtures cannot obtain (justice_appoint refuses is_test accounts).
    // Re-enable once the rls-test-judge / -judge2 / -ag fixtures are
    // provisioned (issue #299; contract: tests/rls/v163.test.ts) and the
    // fixture builder can produce an issued warrant again.
    test.skip(true, 'needs an issued warrant — blocked on the DOJ fixture accounts (issue #299, see tests/rls/v163.test.ts)')
    test.setTimeout(120_000)
    const errs = watch(page)
    ...
    await page.goto(`/legal?request=${fx().judicialQueueWarrant.id}&section=service`)
