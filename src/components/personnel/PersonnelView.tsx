  ...
  return (
    <section className="view-in space-y-6">
      ...
      {state === 'in' && isCommand && (
        ...
        >
          <span>
            <span className="block text-sm font-bold text-white">Manage personnel in the Command Center</span>
      ...
      <div>
        <h2 className="mb-3 text-[13px] font-semibold text-white">Active roster</h2>
  ...
  return (
    <div className={`rounded-lg border bg-ink-900/60 p-5 transition hover:border-white/10 ${p.loa ? 'border-amber-500/20' : 'border-white/5'}`}>
      <div className="flex items-center gap-3">
        <div className="grid h-12 w-12 flex-shrink-0 place-items-center rounded-lg bg-ink-700 text-sm font-bold text-white">
        ...
        <span className={`h-2.5 w-2.5 rounded-full ${p.loa ? 'bg-amber-400' : p.active ? 'bg-emerald-400' : 'bg-slate-500'}`} title={status} />
      </div>
