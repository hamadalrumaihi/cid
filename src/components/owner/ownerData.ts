...
export const SUGGESTIONS: Suggestion[] = [
  ...
  { title: 'Drop or verify bootstrap_command/bootstrap_director RPCs', group: 'Security', why: 'Setup-era privileged functions with no app callers — close the door.', files: 'database (migration)', difficulty: 'S', risk: 'none', benefit: 'medium', verify: 'Security advisors re-run; sign-in unaffected', safeNow: true },
  { title: 'Wire or delete lib/drafts.ts', group: 'Quick win', why: 'Good never-lose-work code with zero importers — dead weight or a free feature.', files: 'src/lib/drafts.ts, chat/report editors', difficulty: 'S', risk: 'none', benefit: 'medium', verify: 'Type a chat draft, reload, draft persists', safeNow: true },
  { title: 'Commit the SQL schema to the repo', group: 'Technical debt', why: 'The live DB is the only source of truth; a schema.sql dump + post-folder migration log makes changes reviewable and recoverable.', files: 'supabase/', difficulty: 'M', risk: 'none', benefit: 'high', verify: 'Dump matches live catalog', safeNow: true },
  { title: 'Split CaseDetail.tsx into per-tab files', group: 'Technical debt', why: 'The hottest, largest file (~840 lines, 12 tabs). Mechanical extraction; keep the RicoTab export for RicoView.', files: 'src/components/cases/CaseDetail.tsx', difficulty: 'M', risk: 'low', benefit: 'high', verify: 'All 12 tabs + packet export + RicoView', safeNow: true },
  { title: 'RLS/RPC test suite (two test users)', group: 'Testing', why: 'The security wall has zero automated coverage — the highest-value testing investment.', files: 'new tests + Supabase branch', difficulty: 'L', risk: 'low', benefit: 'high', verify: 'Cross-bureau denial, sign-off column locks, owner gates', safeNow: true },
  { title: 'Type the JSON columns with zod at read boundaries', group: 'Developer experience', why: 'reports.fields, media.tags, cases.charges, announcement mentions are unchecked casts today.', files: 'src/lib/forms.ts, MediaView, CaseDetail', difficulty: 'M', risk: 'low', benefit: 'medium', verify: 'Old rows still parse; bad shapes surface as toasts', safeNow: true },
  { title: 'Extract a useRegistry hook from the repeated registry skeleton', group: 'Developer experience', why: 'The fetch+version+filter+modal+undo pattern is duplicated ~10×; new registries in minutes.', files: 'registry views', difficulty: 'M', risk: 'medium', benefit: 'medium', verify: 'Migrate one view, diff behavior, then roll on', safeNow: false },
  ...
  { title: 'Keyboard path for board status moves + heat-tint labels', group: 'UX & accessibility', why: 'Drag-and-drop and color-only signals have no keyboard/CVD equivalent in two spots.', files: 'CaseBoard, HeatmapView', difficulty: 'M', risk: 'low', benefit: 'medium', verify: 'Keyboard-only case status change', safeNow: true },
  ...
  { title: 'E2E smoke test (sign-in → create case → sign-off)', group: 'Testing', why: 'Catches integration regressions the four gates cannot.', files: 'new Playwright suite', difficulty: 'L', risk: 'low', benefit: 'high', verify: 'CI runs it against a preview deploy', safeNow: true },
  { title: 'Archive the historical build-era docs', group: 'Documentation', why: 'HANDOFF/ROADMAP/REACT-PARITY/BACKLOG are stale post-cutover and can mislead.', files: 'docs/', difficulty: 'S', risk: 'none', benefit: 'medium', verify: 'Links from README/handbook still resolve', safeNow: true },
]
