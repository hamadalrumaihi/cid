      ...
      <div className="p-5">
        ...
        <div className="space-y-3">
          {drafts.map((row) => <div key={row.id} className="grid gap-2 rounded-xl border border-white/10 bg-ink-950/50 p-3 md:grid-cols-[4rem_1fr_6rem_7rem]">
            ...
            <div className="md:col-span-4 flex justify-end gap-2"><button onClick={() => void saveRow(row)} className="rounded-lg bg-badge-600 px-3 py-2 text-xs font-bold text-white">Save</button><button onClick={() => { void remove('case_templates', row.id).then(() => onChanged()) }} className="rounded-lg border border-rose-400/30 px-3 py-2 text-xs font-bold text-rose-300">Delete</button></div>
          </div>)}
        </div>
