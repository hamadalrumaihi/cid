  ...
  return (
    <Modal open={open} onClose={onClose} wide dirty={dirty}>
      <div className="p-5">
        ...
        {!record && (
          <div className="mb-4 rounded-lg border border-white/10 bg-ink-950/50 p-3">
            <div className="mb-2 flex items-center justify-between gap-2">
              <p className="text-xs font-medium text-slate-500">Templates</p>
              {isCommand && <button onClick={() => setManagerOpen(true)} className="text-xs font-semibold text-badge-200 hover:text-white">Manage</button>}
            </div>
            <div className="flex flex-wrap gap-2">
              <button onClick={() => applyTemplate(null)} className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-semibold text-slate-200">Blank</button>
              {templates.map((tpl) => <button key={tpl.id} onClick={() => applyTemplate(tpl)} className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-semibold text-slate-200 hover:bg-white/10">{tpl.icon || ''} {tpl.name}{tplTasks(tpl).length > 0 && <span className="ml-1 inline-flex items-center gap-0.5 align-middle text-emerald-300"><CheckIcon size={12} />{tplTasks(tpl).length}</span>}</button>)}
            </div>
            {checklist.length > 0 && (
    ...
    <Modal open={open} onClose={onClose} wide>
      <div className="p-5">
        ...
        <div className="space-y-3">
          {drafts.map((row) => <div key={row.id} className="grid gap-2 rounded-lg border border-white/10 bg-ink-950/50 p-3 md:grid-cols-[4rem_1fr_6rem_7rem]">
