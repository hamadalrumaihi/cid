...
export function ChargesTab({ c, canEdit, onChanged }: { c: CaseRow; canEdit: boolean; onChanged: () => void }) {
  ...
  const { run: save, busy } = useAction(async (next: CaseCharge[]) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {canEdit && <div className="rounded-xl border border-white/10 bg-ink-950/50 p-4">
        ...
        {!!recommended.length && <div className="mb-3 flex flex-wrap gap-2">{recommended.map((code) => <button key={code} onClick={() => addCode(code)} disabled={busy} className="rounded-full border border-emerald-400/30 bg-emerald-500/10 px-3 py-1 text-xs font-bold text-emerald-200 disabled:opacity-60">Recommend {code}</button>)}</div>}
        <div className="max-h-72 space-y-2 overflow-y-auto">
          {penalSearch(q).slice(0, 40).map((pc) => <button key={pc.code} onClick={() => addCode(pc.code)} disabled={busy} className="block w-full rounded-lg border border-white/10 bg-white/5 p-3 text-left hover:bg-white/10 disabled:opacity-60"><span className="font-mono text-badge-200">{pc.code}</span> <span className="font-bold text-white">{pc.title}</span><span className="ml-2 text-xs text-slate-500">{pc.level}{pc.rico ? ' - RICO' : ''}</span></button>)}
        </div>
      </div>}
    </div>
  )
