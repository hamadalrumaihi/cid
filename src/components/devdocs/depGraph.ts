...
export const DEP_NODES: DepNode[] = [
  ...
  { id: 'lib/db', kind: 'lib', label: 'lib/db.ts', about: 'THE data layer: list/insert/update/remove/rpc/softDeleteRecord/restoreRecord/withRetry.', dependsOn: ['lib/supabase', 'lib/types', 'lib/toast'], ifChanged: 'The throw-vs-return contract is assumed by ~44 components — check every read try/catch and write res.error path.', risk: 'high' },
  ...
  { id: 'tbl/case-satellites', kind: 'table', label: 'case satellites (evidence, reports, tasks, messages, intel links…)', about: '17 case-scoped tables, all gated by can_access_case.', dependsOn: ['tbl/cases'], ifChanged: 'soft_delete cascades the exclusive children server-side (20261008120001…); deleteRecord (src/lib/deleteRecord.ts) only calls the RPC — check the server cascade map when a satellite FK changes.', risk: 'high' },
