...
function RankCell({ m }: { m: MemberRow }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5">
      ...
      {tier !== 'unknown' && <span className="text-xs font-medium text-slate-500">{tierMeta(tier).label}</span>}
    </span>
  )
}
  ...
  return (
    <div className="space-y-3">
      ...
      {!members.length ? (
        ...
        <p className="rounded-lg border border-white/5 bg-ink-900/60 p-4 text-sm text-slate-400">No members match these filters.</p>
      ) : view === 'table' ? (
          ...
          {tiers.map(({ tier: t, members: rows }) => (
            <div key={t.id}>
              <p className="mb-2 flex items-center gap-2 text-[13px] font-semibold text-white">
                {t.label} <span className="rounded-full bg-white/10 px-1.5 text-[11px] text-slate-400">{rows.length}</span>
              </p>
