/** Provider-neutral integration contracts — the read models the CID domain
 *  will consume from city systems (FiveM servers, MDTs, media hosts) without
 *  depending on any one of them. QB-Core/ESX/ox_inventory/vendor-MDT
 *  specifics live behind adapters that map onto these shapes; nothing in this
 *  directory may reference a concrete framework (spec §2/§3).
 *
 *  Ownership rules (spec §18): city data stays city-authoritative. CID stores
 *  ExternalReference pointers (source + type + id), never a live mirror of
 *  city records. When a copy IS taken (a case needs the state of a record at
 *  seizure time) it is a deliberate, clearly-marked `snapshot` — not an
 *  implicit cache — and it never overwrites the city's copy.
 *
 *  Every field beyond identity is optional: city schemas vary wildly, and an
 *  adapter must be able to return a sparse record rather than fabricate data.
 *  All timestamps are ISO-8601 strings as supplied by the source system. */

/** A configured upstream system. `id` is the stable key every External*
 *  record and reference carries in its `source` field. */
 ...
 *  (when the source provides one) matches this repo's normPlate convention:
 ...
 *  `accessClassification: 'restricted'` maps to CID's break-glass handling. */
