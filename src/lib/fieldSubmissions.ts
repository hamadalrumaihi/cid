 ...
 *  an observation belongs to a bureau or to the Special Investigations Bureau,
 *  and asking produced a guess or "unsure". Where they were standing, they
 *  know.
 *
 *  It is deliberately NOT inferred from the reporting agency. SAHP is
 *  statewide — a trooper works both.
 *
 *  Jurisdiction is a FACT about the observation, not a routing key: the old
 *  geographic city/blaine → bureau queue mapping died with the bureau
...
export function submitProblem(s: Pick<FieldSubmissionRow,
  ...
  // Required by a check constraint too — where it happened is a fact every
  // reviewer needs, even though it no longer routes the report anywhere.
