...
| **Triggers** | The non-definer freeze triggers enumerate columns explicitly; a new privileged column is unprotected until added (`block_direct_privileged_profile`, `block_direct_login_denied`, `block_direct_report_finalize`, `block_direct_signoff`, `block_direct_case_bureau`) | Adding a privileged column? Add it to the freeze trigger **in the same migration** |
