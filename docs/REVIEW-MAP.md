...
**Navigation model**: one dynamic route, `src/app/(app)/[tab]/page.tsx`, statically prerendered per leaf tab from `PAGE_META` in `src/lib/nav.ts` — so "UI entry" below is a real `/{tab}` path. The former Intelligence category's 14 tabs are consolidated behind one nav item, `/tools` (**Investigative Tools**: `src/components/tools/ToolsView.tsx`, model in `src/lib/toolsModel.ts`) — their legacy `/{tab}` routes stay prerendered and redirect into the workspace (`ToolTabRedirect`, query params carried over), so the paths below remain valid entries; each tool renders the same RLS-scoped view inside the workspace, and permissions are unchanged. `src/app/(app)/layout.tsx` gates everything: signed-out → `Gate`; justice-only identity → the standalone `JusticeShell` (no CID nav); otherwise `AppShell` + `Sidebar` (Command Center leaf for command/Owner, Justice leaf for justice roles/Owner, Owner leaf Owner-only).
...
| Announcements | `/announce` | `announce/{AnnounceView,AnnouncementModal}.tsx` | `announcements` (audience ∈ `all/command/specific_members/major_crimes/street_crimes/JTF`), `notifications` | `publish_announcement`, `announcement_recipient_count`, `announcement_notify_update` | `private.can_announce`, `can_post_audience`, `announcement_recipients` | `rls.test.ts` (audience authority); e2e `features-joint-announce` |
...
| Vehicles / Persons / Gangs intel | `/tools` (Investigative Tools workspace; legacy `/vehicles`, `/persons`, `/gangs`, `/bolo`, `/places`, `/indicators`, `/network`, `/narcotics`, `/ballistics`, `/modus` redirect in) | `tools/{ToolsView,toolRegistry}.tsx` hosting `vehicles/VehiclesView.tsx`, `persons/{PersonsView,IntelProfile,PersonProfile}.tsx`, `gangs/GangsView.tsx` etc. | `vehicles`, `persons`, `gangs(+members/ranks/turf)`, `places`, `indicators`, `mo_profiles`, `case_intel_links` | `mo_crossref` (existence-only cross-bureau matches) | `private.is_active` (read/write), `can_delete` (command delete) | generic wall in `rls.test.ts` |
...
| Special Investigations Bureau (SIB) | `/siu` (standalone sidebar leaf, rendered only with SIB standing) | `siu/SiuView.tsx`, `siu/SiuDisclosures.tsx`, `siu/SiuTradecraft.tsx`; `cases/SiuControlBar.tsx` (§14 takeover), `cases/ReleasedIntelligence.tsx` (§15, CID side); `src/lib/siu.ts` (capability mirror), `src/lib/useSiu.ts` (standing hook); SIB cases open in the ordinary case workspace | `siu_settings`, `siu_memberships`, `siu_case_agents`, `siu_compartment_members`, `siu_disclosures`, `siu_sources`, `siu_undercover_operations`, `siu_financial_intel`, `siu_comms_intel`, `siu_integrity_reviews`, `siu_exports`, `cases.case_authority` / `siu_classification` / `siu_assumed_*` / `siu_returned_at` | `siu_set_release`, `siu_appoint`, `siu_remove`, `siu_set_callsign`, `siu_create_case`, `siu_set_case_classification`, `siu_assign_agent`, `siu_unassign_agent`, `siu_compartment_add`, `siu_compartment_remove`, `siu_roster`, `siu_member_search`, `siu_audit_feed`, `siu_overview`, `next_siu_case_number`, `siu_assume_control`, `siu_release_control`, `siu_share`, `siu_revoke_disclosure`, `siu_acknowledge_disclosure`, `siu_released_intelligence`, `siu_export_case`, `siu_oversight_report` | `private.siu_standing`, `siu_operates`, `siu_is_agent/_is_command/_can_appoint`, `siu_case_access` (write wall), `siu_case_read` (read superset — oversight sees standard investigations), `siu_case_command`, `siu_in_compartment`, `siu_oversight_read`, `is_siu_case`, `can_delete_case_child`/`can_delete_case_file` (SIB case-child delete wall); chokepoints `can_access_case(_row)` + read-only superset `can_read_case(_row/_number)`; trigger `block_direct_siu_case_cols` | `src/lib/siu.test.ts` (capability mirror), `tests/rls/v166.test.ts` (live wall: CID→SIB denial, RPC refusal, compartments, CID regression), `tests/rls/v167.test.ts` (live: §14 takeover + return, §15 addressing and origin secrecy, Phase 3 invisibility, export redaction) |
...
- **Self-recusal beats seniority (SIB §17)**: `siu_declare_conflict()` inserts a
  row that `private.siu_recused()` reads as a hard veto, checked *first* in
  `siu_case_access()` — above rank, above SIB command, above `owner`. Only a
  different member of SIB command can lift it (`siu_resolve_conflict()` refuses
  the declarer), and only the `cleared` status restores access.
- **The SIB intake queue is closed to oversight**: `siu_referrals` is gated on
  `siu_is_agent()`, not on SIB standing generally, because a referral can name
...
- **A case-child DELETE needs case ACCESS, not just a rank**: `private.can_delete()`
  ...
  including, across the departmental wall, an SIB member on a CID case.
...
- **§30 supporting access is not SIB standing**: a grant opens ONE investigation's
