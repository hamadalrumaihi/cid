...
| Special Investigation Unit (SIU) | `/siu` (standalone sidebar leaf, rendered only with SIU standing) | `siu/SiuView.tsx`, `siu/SiuDisclosures.tsx`, `siu/SiuTradecraft.tsx`; `cases/SiuControlBar.tsx` (§14 takeover), `cases/ReleasedIntelligence.tsx` (§15, CID side); `src/lib/siu.ts` (capability mirror), `src/lib/useSiu.ts` (standing hook); SIU cases open in the ordinary case workspace | `siu_settings`, `siu_memberships`, `siu_case_agents`, `siu_compartment_members`, `siu_disclosures`, `siu_sources`, `siu_undercover_operations`, `siu_financial_intel`, `siu_comms_intel`, `siu_integrity_reviews`, `siu_exports`, `cases.case_authority` / `siu_classification` / `siu_assumed_*` / `siu_returned_at` | `siu_set_release`, `siu_appoint`, `siu_remove`, `siu_set_callsign`, `siu_create_case`, `siu_set_case_classification`, `siu_assign_agent`, `siu_unassign_agent`, `siu_compartment_add`, `siu_compartment_remove`, `siu_roster`, `siu_member_search`, `siu_audit_feed`, `siu_overview`, `next_siu_case_number`, `siu_assume_control`, `siu_release_control`, `siu_share`, `siu_revoke_disclosure`, `siu_acknowledge_disclosure`, `siu_released_intelligence`, `siu_export_case`, `siu_oversight_report` | `private.siu_standing`, `siu_operates`, `siu_is_agent/_is_command/_can_appoint`, `siu_case_access` (write wall), `siu_case_read` (read superset — oversight sees standard investigations), `siu_case_command`, `siu_in_compartment`, `siu_oversight_read`, `is_siu_case`, `can_delete_case_child`/`can_delete_case_file` (SIU case-child delete wall); chokepoints `can_access_case(_row)` + read-only superset `can_read_case(_row/_number)`; trigger `block_direct_siu_case_cols` | `src/lib/siu.test.ts` (capability mirror), `tests/rls/v166.test.ts` (live wall: CID→SIU denial, RPC refusal, compartments, CID regression), `tests/rls/v167.test.ts` (live: §14 takeover + return, §15 addressing and origin secrecy, Phase 3 invisibility, export redaction) |
