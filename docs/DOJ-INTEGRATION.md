...
| `WorkflowTimeline` — extracted to `ui/WorkflowTimeline.tsx` | legal action history (`LegalRequestDetail` History tab) | **Adopted v1.14:** case sign-off history (SignoffTab), evidence custody chain (Evidence & Media tab, MediaTab), Command Center approval-queue history, CID + Justice membership-request applicant history panels | none — shipped |
