...
Audit: `RECORD_SOFT_DELETED` / `RECORD_RESTORED` rows (entity = table, detail `{kind, reason, batch, cascaded|restored}`) on top of the tables' own `private.audit()` `UPDATE` rows. Client: `src/lib/db.ts` filters these tables to live rows in `list()` / `countRows()` (`includeDeleted` opts in), routes `remove()` and `deleteWithUndo()` through `soft_delete` (with a reason prompt for the parent kinds) and backs the Undo toast with `restore_record`. Pinned by `v180a`.
