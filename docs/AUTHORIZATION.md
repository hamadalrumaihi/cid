...
### §25 — The watchlist ([same migration](../supabase/migrations/20260831120000_siu_intelligence_quality.sql))

Unit-level rather than case-level. Two rules give it a spine:

- **Expiry is mandatory.** `expires_at` is NOT NULL and capped at 365 days per grant, and `private.siu_watch_live()` evaluates it **against the clock**, not against the status column — a stale `active` row cannot keep a watch alive past its end date, and no sweeper job has to run. Extending is a separate, reasoned, audited act. A watch entry with no end date is a permanent secret dossier on a named person.
...
Removal keeps the row. Who was watched, why, and who stopped it is what makes a watchlist accountable rather than a private list.
...
| **SIU intelligence quality & watchlist** | notes: `siu_can_read_case_note(case_id)`; watchlist: SIU **field agents only** (`siu_is_agent`) — never oversight, which the list may name | watch entries: field standing (`siu_watch_add`); grading is set at note authorship | grading RPC-only after INSERT, review columns RPC-only always (trigger `block_direct_siu_note_grading`); a watch is extended only by a reasoned `siu_watch_extend` | — | watch entries are **removed, not deleted** — the record of who was watched is the accountability | `siu_grade_note`, `siu_review_note`, `siu_intel_quality`, `siu_watch_add`/`_extend`/`_remove`, `siu_deconflict` | `siu_watchlist_sel` → `siu_is_agent`; `private.siu_watch_live()` reads expiry off the clock |
