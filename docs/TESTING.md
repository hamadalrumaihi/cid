...
The platform-upgrade suites `v192a` (evidence), `v192b` (packets / documents / jobs) and `v192c` (sources / graph / search + **the CI proof** across every new surface) follow the `v191` pattern and are documented in [`tests/rls/README.md`](../tests/rls/README.md). Offline, `src/lib/urlPolicy.test.ts` pins every SSRF class and the byte-identity of the three URL-policy copies, `src/lib/jobsModel.test.ts` the retry formula, and `src/mocks/handlers/platform.ts` the offline contract of every new RPC, table wall, bucket and query function.
