...
Welcome to the **CID Portal**, the live investigative case-management system for the San Andreas Criminal Investigation Division and the Special Investigations Bureau. This is the operational manual: what each workspace is, how to get access, and how to run your work day to day.
...
- **Associations — one organization to another, and to a property.** When two organizations turn up in the same place, or you hear that one works for another, record it on either dossier's **Associations** section. You pick the other record, the claim (**Unconfirmed Association** when all you have is that they are somehow connected, or something specific like *rivalry*, *shared property*, *business relationship*, *splinter*) and a note. **The row always starts at Pending Investigation** — recording it says you saw something, not that it is true, and an Unconfirmed Association never means the two are allies, merged or under one command. Later, anyone who can see both records can **Confirm**, **Reject** or **Mark historical** with a reason, and correct the claim at the same time; **Reopen** puts it back to pending and clears the decision. The person who recorded it is shown as the **submitter**, which is not the same as being assigned to investigate it. Only you or command may amend or withdraw your own row — but withdrawing says it should never have been recorded, so if you considered it and decided against it, **Reject** it instead and the reasoning stays on the record. Recording the same association twice, in either direction, just shows you the row that is already there; nothing is duplicated and nothing you wrote is overwritten. Associations go to the Trash and come back like any other record.
- **Photographs on a registry record.** A gang, person, place, vehicle or narcotics record takes **photographs** — a tag, a mural, a shopfront, a patch. **Attach photograph** uploads the original into the portal's own private storage, and it is served only through short-lived links to people who may already see the record. This is *intelligence*, not case evidence: it gets no `EV-` number, no chain of custody and no integrity check, and it cannot be turned into evidence later — evidence belongs to a case and is registered there. Attaching a second photograph adds to the record; it never replaces the first.
