...
3. First time here? After signing in you'll apply for a **CID** role. (Retired 2026-07-22: the separate **DOJ**/**Judiciary** signup path is removed — legal approval is now handled by CID Command, so there are no prosecutor/Judge applications. Older accounts describe a DOJ/Judiciary choice; that no longer applies.)
   - **CID applicants** fill in display name, badge number, your **one permanent department** (Los Santos, Blaine County or State Bureau — JTF is a temporary joint-case designation, assigned per case, never a permanent department), the CID role you're requesting (any normal role, **Detective through Director** — requesting grants nothing, and higher roles need higher approvers, up to the Owner for Director), and a short reason — then **Submit Request**. Command reviews it. Once approved, your assigned role and department stay in effect until an authorized promotion, demotion, or transfer is completed — another request or a profile edit can't change them, and a department move is made directly by an authorized leader — a Bureau Lead when the move touches their own bureau, or Deputy Director and above — and takes effect immediately.
   The reviewer can approve as-is, approve with a different role, send it back for a correction (you'll see their note, edit, and resubmit), or reject it. You stay locked out until a request is approved; once it is, reload and you're in — everyone works in the full CID portal.
...
1. **File Warrant Request** or **File Subpoena** (or, on a finalized arrest-warrant report, click **Submit for Legal Review**). Pick the case, the suspect/recipient from the Persons registry (warrants and player subpoenas) or type a business/entity name, fill the justification, and — for subpoenas — the type-specific fields.
   - **Choose the Warrant Type** on a warrant request: **Arrest Warrant** or **Search Warrant**. An arrest warrant needs a suspect from the Persons registry. A **search warrant** targets a person **and/or** one or more places, properties, postal areas or vehicles — so the subject is **optional**; instead fill the **Search Targets** (one per line — required) and **Items Sought** (required), plus **Place to Search** and **Vehicle Targets** as needed. The justification you write is the probable cause. You must give a subject **or** at least one search target.
...
3. **Submit for CID review.** A **packet preview** opens first: a requirements checklist, your selected items cross-checked against their live sources (anything missing or not yet finalized is flagged), and a reminder that reviewers receive **only the packet** — never access to the rest of the case. Confirm to submit.
4. **A Bureau Lead (or above) reviews it** and approves, denies, or returns it for revision. This is the whole approval — **both warrants and subpoenas terminate at Bureau Lead+ approval; there is no separate Judge, ADA, DA, or AG step.** Approval authorizes applying the warrant/subpoena in-city; it does **not** auto-activate it. You'll get a notification on the decision and can watch progress on the request's **Review History** tab.
5. **Once approved, you issue and execute/serve it.** An authorized CID officer records **issue**, then **execution/return** (warrants) or **service/compliance** (subpoenas) on the **Fulfilment** tab. Materials received under a subpoena are logged back to the case as evidence/attachments. For BOLOs/wanted persons, an approved arrest warrant can be pushed to the in-city **MDT** via the Lead+-approved export.
...
All legal work now lives in the CID **Legal** surface (the **Legal Requests** tab and each case's **Legal** tab) — there is no separate Justice portal. Sealed requests are visible only to the people directly involved.

> **What changed (retired 2026-07-22):** legal requests no longer route through the DOJ or a Judge. Historical warrants and subpoenas that were decided by a Judge, DA, or AG before this change are preserved and still viewable read-only inside each request's dossier.
...
| Clicked **Delete** by accident | Most deletions show an **Undo** toast for a few seconds. Click it. Cases are never deleted this way — Command **archives** them instead (archived cases can be restored), and only the portal owner can permanently delete a case. If something else is gone, ask Command — records are audit-logged. |
