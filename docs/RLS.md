...
The suites in [`tests/rls/`](../tests/rls/) (`rls.test.ts`, `legal.test.ts`, `v114`–`v129.test.ts`) sign in to the **live project** as the low-privilege fixtures using the anon key + password grant — **never a service key** — and assert both directions:
