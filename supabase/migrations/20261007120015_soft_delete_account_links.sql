-- ============================================================================
-- Soft delete — Account–person links (public.account_links). Portal Improvements plan,
-- Phase 1, P1-03a; one file per table so a single policy can be re-emitted
-- or reverted on its own (plan §6.2b).
--
-- Purpose
--   Replace the client DELETE on public.account_links with a soft delete: the row
...
--
-- Caller
--   RLS (every client read/update of account_links); the lifecycle columns are
...
--   account_links_del as it was and re-emit the two policies without the
--   conjunct (the columns can stay).
--
-- APPLICATION NOTE: applied live as soft_delete_account_links.
-- ============================================================================
...
create index if not exists account_links_deleted_at_idx on public.account_links (deleted_at) where deleted_at is not null;
create index if not exists account_links_delete_batch_idx on public.account_links (delete_batch) where delete_batch is not null;

drop trigger if exists account_links_block_direct_soft_delete on public.account_links;
create trigger account_links_block_direct_soft_delete
...
create policy account_links_sel on public.account_links
  ...
  using ((private.is_live(deleted_at) or private.is_owner()) and ((private.is_active() AND (NOT private.siu_blocked('account'::text, account_id, 'accounts'::text)) AND (NOT private.siu_blocked('person'::text, person_id, 'accounts'::text)))));
...
create policy account_links_upd on public.account_links
  ...
  using ((private.is_live(deleted_at) or private.is_owner()) and ((private.is_active() AND (NOT private.siu_blocked('account'::text, account_id, 'accounts'::text)) AND (NOT private.siu_blocked('person'::text, person_id, 'accounts'::text)))))
  with check ((private.is_live(deleted_at) or private.is_owner()) and ((private.is_active() AND (NOT private.siu_blocked('account'::text, account_id, 'accounts'::text)) AND (NOT private.siu_blocked('person'::text, person_id, 'accounts'::text)))));

drop policy if exists account_links_del on public.account_links;
revoke delete on public.account_links from anon, authenticated;
