...
| `20260615120200_seed_catalogs.sql` | Seeded `report_templates` + `rico_predicate_catalog` tables. **The live app ships these as client-side constants** (now `FORM_SCHEMAS` / `REPORT_TEMPLATES` in `src/lib/forms.ts`); neither table exists in production. RICO uses the live `rico_cases` + `predicate_acts` tables instead. Nothing to rebase. |
