-- ============================================================================
-- Soft delete — GPS trackers (public.trackers). Portal Improvements plan,
-- Phase 1, P1-03a; one file per table so a single policy can be re-emitted
-- or reverted on its own (plan §6.2b).
--
-- Purpose
--   Replace the client DELETE on public.trackers with a soft delete: the row
...
--
-- Caller
--   RLS (every client read/update of trackers); the lifecycle columns are
...
--   trackers_del as it was and re-emit the two policies without the
--   conjunct (the columns can stay).
--
-- APPLICATION NOTE: applied live as soft_delete_trackers.
-- ============================================================================
...
create index if not exists trackers_deleted_at_idx on public.trackers (deleted_at) where deleted_at is not null;
create index if not exists trackers_delete_batch_idx on public.trackers (delete_batch) where delete_batch is not null;

drop trigger if exists trackers_block_direct_soft_delete on public.trackers;
create trigger trackers_block_direct_soft_delete
  before insert or update on public.trackers
  for each row execute function private.block_direct_soft_delete();

drop policy if exists trackers_sel on public.trackers;
...
create policy trackers_upd on public.trackers
  ...
  using ((private.is_live(deleted_at) or private.is_owner()) and (private.can_delete()))
  with check ((private.is_live(deleted_at) or private.is_owner()) and (private.can_delete()));

drop policy if exists trackers_del on public.trackers;
revoke delete on public.trackers from anon, authenticated;
