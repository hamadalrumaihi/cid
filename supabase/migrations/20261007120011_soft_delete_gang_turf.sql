-- ============================================================================
-- Soft delete — Gang turf (public.gang_turf). Portal Improvements plan,
-- Phase 1, P1-03a; one file per table so a single policy can be re-emitted
-- or reverted on its own (plan §6.2b).
--
-- Purpose
--   Replace the client DELETE on public.gang_turf with a soft delete: the row
...
--
-- Caller
--   RLS (every client read/update of gang_turf); the lifecycle columns are
...
--   gang_turf_del as it was and re-emit the two policies without the
--   conjunct (the columns can stay).
--
-- APPLICATION NOTE: applied live as soft_delete_gang_turf.
-- ============================================================================
...
create index if not exists gang_turf_deleted_at_idx on public.gang_turf (deleted_at) where deleted_at is not null;
create index if not exists gang_turf_delete_batch_idx on public.gang_turf (delete_batch) where delete_batch is not null;

drop trigger if exists gang_turf_block_direct_soft_delete on public.gang_turf;
create trigger gang_turf_block_direct_soft_delete
  before insert or update on public.gang_turf
  for each row execute function private.block_direct_soft_delete();

drop policy if exists gang_turf_sel on public.gang_turf;
create policy gang_turf_sel on public.gang_turf
  for select to authenticated
  using ((private.is_live(deleted_at) or private.is_owner()) and ((private.is_active() AND (NOT private.siu_blocked('gang'::text, gang_id, 'gang_turf'::text)))));

drop policy if exists gang_turf_upd on public.gang_turf;
create policy gang_turf_upd on public.gang_turf
  for update to authenticated
  using ((private.is_live(deleted_at) or private.is_owner()) and ((private.is_active() AND (NOT private.siu_blocked('gang'::text, gang_id, 'gang_turf'::text)))))
  with check ((private.is_live(deleted_at) or private.is_owner()) and ((private.is_active() AND (NOT private.siu_blocked('gang'::text, gang_id, 'gang_turf'::text)))));

drop policy if exists gang_turf_del on public.gang_turf;
revoke delete on public.gang_turf from anon, authenticated;
