-- ============================================================================
-- Soft delete — Gangs registry (public.gangs). Portal Improvements plan,
-- Phase 1, P1-03a; one file per table so a single policy can be re-emitted
-- or reverted on its own (plan §6.2b).
--
-- Purpose
--   Replace the client DELETE on public.gangs with a soft delete: the row
...
--
-- Caller
--   RLS (every client read/update of gangs); the lifecycle columns are
...
--   gangs_del as it was and re-emit the two policies without the
--   conjunct (the columns can stay).
--
-- APPLICATION NOTE: applied live as soft_delete_gangs.
-- ============================================================================
...
create index if not exists gangs_deleted_at_idx on public.gangs (deleted_at) where deleted_at is not null;
create index if not exists gangs_delete_batch_idx on public.gangs (delete_batch) where delete_batch is not null;

drop trigger if exists gangs_block_direct_soft_delete on public.gangs;
create trigger gangs_block_direct_soft_delete
  before insert or update on public.gangs
...
revoke delete on public.gangs from anon, authenticated;
