-- ============================================================================
-- Soft delete — Places registry (public.places). Portal Improvements plan,
-- Phase 1, P1-03a; one file per table so a single policy can be re-emitted
-- or reverted on its own (plan §6.2b).
--
-- Purpose
--   Replace the client DELETE on public.places with a soft delete: the row
...
--
-- Caller
--   RLS (every client read/update of places); the lifecycle columns are
...
--   places_del as it was and re-emit the two policies without the
--   conjunct (the columns can stay).
--
-- APPLICATION NOTE: applied live as soft_delete_places.
-- ============================================================================
...
create index if not exists places_deleted_at_idx on public.places (deleted_at) where deleted_at is not null;
create index if not exists places_delete_batch_idx on public.places (delete_batch) where delete_batch is not null;

drop trigger if exists places_block_direct_soft_delete on public.places;
create trigger places_block_direct_soft_delete
...
revoke delete on public.places from anon, authenticated;
