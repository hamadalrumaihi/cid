-- ============================================================================
-- Soft delete — Accounts registry (public.accounts). Portal Improvements plan,
-- Phase 1, P1-03a; one file per table so a single policy can be re-emitted
-- or reverted on its own (plan §6.2b).
--
-- Purpose
--   Replace the client DELETE on public.accounts with a soft delete: the row
...
--
-- Caller
--   RLS (every client read/update of accounts); the lifecycle columns are
...
--   accounts_del as it was and re-emit the two policies without the
--   conjunct (the columns can stay).
--
-- APPLICATION NOTE: applied live as soft_delete_accounts.
-- ============================================================================
...
create index if not exists accounts_deleted_at_idx on public.accounts (deleted_at) where deleted_at is not null;
create index if not exists accounts_delete_batch_idx on public.accounts (delete_batch) where delete_batch is not null;

drop trigger if exists accounts_block_direct_soft_delete on public.accounts;
create trigger accounts_block_direct_soft_delete
