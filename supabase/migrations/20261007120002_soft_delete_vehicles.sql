-- ============================================================================
-- Soft delete — Vehicles registry (public.vehicles). Portal Improvements plan,
-- Phase 1, P1-03a; one file per table so a single policy can be re-emitted
-- or reverted on its own (plan §6.2b).
--
-- Purpose
--   Replace the client DELETE on public.vehicles with a soft delete: the row
...
--
-- Caller
--   RLS (every client read/update of vehicles); the lifecycle columns are
...
--   vehicles_del as it was and re-emit the two policies without the
--   conjunct (the columns can stay).
--
-- APPLICATION NOTE: applied live as soft_delete_vehicles.
-- ============================================================================
...
create index if not exists vehicles_delete_batch_idx on public.vehicles (delete_batch) where delete_batch is not null;

drop trigger if exists vehicles_block_direct_soft_delete on public.vehicles;
create trigger vehicles_block_direct_soft_delete
...
create policy vehicles_sel on public.vehicles
  ...
  using ((private.is_live(deleted_at) or private.is_owner()) and ((private.is_active() AND (NOT private.siu_hidden('vehicle'::text, id)))));

drop policy if exists vehicles_upd on public.vehicles;
create policy vehicles_upd on public.vehicles
  for update to authenticated
  using ((private.is_live(deleted_at) or private.is_owner()) and ((private.is_active() AND (NOT private.siu_hidden('vehicle'::text, id)))))
  with check ((private.is_live(deleted_at) or private.is_owner()) and ((private.is_active() AND (NOT private.siu_hidden('vehicle'::text, id)))));
