-- ============================================================================
-- Soft delete — Gang roster links (public.gang_members). Portal Improvements plan,
-- Phase 1, P1-03a; one file per table so a single policy can be re-emitted
-- or reverted on its own (plan §6.2b).
--
-- Purpose
--   Replace the client DELETE on public.gang_members with a soft delete: the row
...
--
-- Caller
--   RLS (every client read/update of gang_members); the lifecycle columns are
...
--   gang_members_del as it was and re-emit the two policies without the
--   conjunct (the columns can stay).
--
-- APPLICATION NOTE: applied live as soft_delete_gang_members.
-- ============================================================================
...
create index if not exists gang_members_deleted_at_idx on public.gang_members (deleted_at) where deleted_at is not null;
create index if not exists gang_members_delete_batch_idx on public.gang_members (delete_batch) where delete_batch is not null;

drop trigger if exists gang_members_block_direct_soft_delete on public.gang_members;
create trigger gang_members_block_direct_soft_delete
  before insert or update on public.gang_members
  for each row execute function private.block_direct_soft_delete();

drop policy if exists gang_members_sel on public.gang_members;
create policy gang_members_sel on public.gang_members
  for select to authenticated
  using ((private.is_live(deleted_at) or private.is_owner()) and ((private.is_active() AND (NOT private.siu_blocked('gang'::text, gang_id, 'gang_membership'::text)) AND (NOT private.siu_blocked('person'::text, person_id, 'gang_membership'::text)))));

drop policy if exists gang_members_upd on public.gang_members;
create policy gang_members_upd on public.gang_members
  for update to authenticated
  using ((private.is_live(deleted_at) or private.is_owner()) and ((private.is_active() AND (NOT private.siu_blocked('gang'::text, gang_id, 'gang_membership'::text)) AND (NOT private.siu_blocked('person'::text, person_id, 'gang_membership'::text)))))
  with check ((private.is_live(deleted_at) or private.is_owner()) and ((private.is_active() AND (NOT private.siu_blocked('gang'::text, gang_id, 'gang_membership'::text)) AND (NOT private.siu_blocked('person'::text, person_id, 'gang_membership'::text)))));

drop policy if exists gang_members_del on public.gang_members;
revoke delete on public.gang_members from anon, authenticated;
