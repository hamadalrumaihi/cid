-- ============================================================================
-- Soft delete — Case intelligence links (public.case_intel_links). Portal Improvements plan,
...
--
-- Purpose
--   Replace the client DELETE on public.case_intel_links with a soft delete: the row
...
--
-- Caller
--   RLS (every client read/update of case_intel_links); the lifecycle columns are
...
--   Rollback for this table: re-create case_intel_links_del as it was and re-emit
--   the two policies without the conjunct.
--
-- APPLICATION NOTE: applied live as soft_delete_case_intel_links.
-- ============================================================================

alter table public.case_intel_links
  add column if not exists deleted_at timestamptz,
  add column if not exists deleted_by uuid references public.profiles(id),
  add column if not exists delete_reason text,
  add column if not exists delete_batch uuid;
create index if not exists case_intel_links_deleted_at_idx on public.case_intel_links (deleted_at) where deleted_at is not null;
create index if not exists case_intel_links_delete_batch_idx on public.case_intel_links (delete_batch) where delete_batch is not null;

drop trigger if exists case_intel_links_block_direct_soft_delete on public.case_intel_links;
create trigger case_intel_links_block_direct_soft_delete
  before insert or update on public.case_intel_links
...
drop policy if exists case_intel_links_del on public.case_intel_links;
revoke delete on public.case_intel_links from anon, authenticated;
