-- ============================================================================
-- Soft delete — RICO cases (public.rico_cases). Portal Improvements plan,
...
--
-- Purpose
--   Replace the client DELETE on public.rico_cases with a soft delete: the row
...
--
-- Caller
--   RLS (every client read/update of rico_cases); the lifecycle columns are
...
--   Rollback for this table: re-create rico_cases_del as it was and re-emit
--   the two policies without the conjunct.
--
-- APPLICATION NOTE: applied live as soft_delete_rico_cases.
-- ============================================================================

alter table public.rico_cases
  add column if not exists deleted_at timestamptz,
  add column if not exists deleted_by uuid references public.profiles(id),
  add column if not exists delete_reason text,
  add column if not exists delete_batch uuid;
create index if not exists rico_cases_deleted_at_idx on public.rico_cases (deleted_at) where deleted_at is not null;
create index if not exists rico_cases_delete_batch_idx on public.rico_cases (delete_batch) where delete_batch is not null;

drop trigger if exists rico_cases_block_direct_soft_delete on public.rico_cases;
create trigger rico_cases_block_direct_soft_delete
  before insert or update on public.rico_cases
...
revoke delete on public.rico_cases from anon, authenticated;
