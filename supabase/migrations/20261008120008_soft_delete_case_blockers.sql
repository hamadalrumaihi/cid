-- ============================================================================
-- Soft delete — Case blockers (public.case_blockers). Portal Improvements plan,
...
--
-- Purpose
--   Replace the client DELETE on public.case_blockers with a soft delete: the row
...
--
-- Caller
--   RLS (every client read/update of case_blockers); the lifecycle columns are
...
--   Rollback for this table: re-create case_blockers_del as it was and re-emit
--   the two policies without the conjunct.
--
-- APPLICATION NOTE: applied live as soft_delete_case_blockers.
-- ============================================================================

alter table public.case_blockers
  add column if not exists deleted_at timestamptz,
  add column if not exists deleted_by uuid references public.profiles(id),
  add column if not exists delete_reason text,
  add column if not exists delete_batch uuid;
create index if not exists case_blockers_deleted_at_idx on public.case_blockers (deleted_at) where deleted_at is not null;
create index if not exists case_blockers_delete_batch_idx on public.case_blockers (delete_batch) where delete_batch is not null;

drop trigger if exists case_blockers_block_direct_soft_delete on public.case_blockers;
create trigger case_blockers_block_direct_soft_delete
  before insert or update on public.case_blockers
...
revoke delete on public.case_blockers from anon, authenticated;
