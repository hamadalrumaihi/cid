-- ============================================================================
-- Soft delete — Indicators registry (public.indicators). Portal Improvements plan,
-- Phase 1, P1-03a; one file per table so a single policy can be re-emitted
-- or reverted on its own (plan §6.2b).
--
-- Purpose
--   Replace the client DELETE on public.indicators with a soft delete: the row
...
--
-- Caller
--   RLS (every client read/update of indicators); the lifecycle columns are
...
--   indicators_del as it was and re-emit the two policies without the
--   conjunct (the columns can stay).
--
-- APPLICATION NOTE: applied live as soft_delete_indicators.
-- ============================================================================
...
create index if not exists indicators_deleted_at_idx on public.indicators (deleted_at) where deleted_at is not null;
create index if not exists indicators_delete_batch_idx on public.indicators (delete_batch) where delete_batch is not null;

drop trigger if exists indicators_block_direct_soft_delete on public.indicators;
create trigger indicators_block_direct_soft_delete
  before insert or update on public.indicators
  for each row execute function private.block_direct_soft_delete();

drop policy if exists indicators_sel on public.indicators;
create policy indicators_sel on public.indicators
  for select to authenticated
  using ((private.is_live(deleted_at) or private.is_owner()) and ((private.is_active() AND (NOT private.siu_blocked('indicator'::text, id, NULL::text)))));

drop policy if exists indicators_upd on public.indicators;
create policy indicators_upd on public.indicators
  for update to authenticated
  using ((private.is_live(deleted_at) or private.is_owner()) and ((private.is_active() AND (NOT private.siu_blocked('indicator'::text, id, NULL::text)))))
  with check ((private.is_live(deleted_at) or private.is_owner()) and ((private.is_active() AND (NOT private.siu_blocked('indicator'::text, id, NULL::text)))));

drop policy if exists indicators_del on public.indicators;
revoke delete on public.indicators from anon, authenticated;
