-- ============================================================================
-- rls_test_cleanup() must sweep the Delivery B tables too.
--
-- 20260831120000 added public.siu_watchlist and 20260831130000 added
-- public.siu_temporary_access. Neither was in the sweep.
--
-- The temporary-access row is the one that matters. A §30 grant is a LIVE KEY
-- into an SIU investigation. A fixture holding one on a real case would keep
-- reading that file every run until it expired -- and the fixture's password is
-- a GitHub Actions secret. It is removed and reported, without exception.
--
-- The watchlist follows the referral rule from 20260830140000: a fixture entry
-- naming a real officer is a live watch nobody chose to place, readable by
-- every SIU field agent, so removing it is the only defensible outcome and the
-- escape is still reported.
--
-- Ordering note: both deletes run BEFORE `delete from public.cases`, so a
-- fixture case's grants and watches are gone before the cascade would take
-- them silently -- which keeps the returned counts honest.
--
-- Each delete gets its OWN counter. The first cut shared one `get diagnostics`
-- across both, so the grants were swept and reported as zero. A cleanup that
-- removes rows without saying so is precisely the failure the `leaked` array
-- exists to prevent, and a miscounted total is the same failure in miniature.
--
-- ADDITIVE ONLY: one function body re-emitted. Everything outside the new block
-- and its counter is verbatim from 20260830140000.
--
-- APPLICATION NOTE: applied live as rls_cleanup_siu_delivery_b.
-- ============================================================================
...
begin
  ...
  -- Delivery B (20260831120000 / 20260831130000). A fixture TEMPORARY ACCESS
  -- grant on a REAL SIU investigation is a live key into that file held by an
  -- account whose password is a CI secret. It goes, and it is reported loudly.
  ...
  -- A fixture watch entry can NAME a real person. Same rule as a referral:
  -- leaving it means a live watch on a real officer, readable by every field
  -- agent, that nobody chose to place.
  ...
  return jsonb_build_object('cases', n_cases, 'reports', n_reports, 'evidence', n_evidence,
    ...
    'siu_referrals', n_siu_intake, 'siu_watchlist', n_siu_watch,
    'siu_temporary_access', n_siu_grants,
    'leaked', leaked);
end $$;

-- ============================================================================
-- Rollback: re-emit public.rls_test_cleanup() from
-- 20260830140000_rls_cleanup_siu_intake.sql. Doing so stops cleaning
-- siu_watchlist / siu_temporary_access, so fixture watches and — worse —
-- fixture access grants will accumulate against live investigations.
-- ============================================================================
