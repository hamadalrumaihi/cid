...
| `public.ci_context()` / `ci_list` / `ci_get` / `ci_stats` / `ci_search` / `ci_person_status` / `ci_case_intel` / `ci_case_counts` / `ci_audit_list` / `ci_export` | The Confidential Informant compartment — reads, all behind `private.can_access_ci` (= `has_full_ci_access` **or** an active handler row): a caller who is not authorized gets null / zero rows (never a raise, a count or a "denied" — existence is never confirmed); `ci_context()` is exactly `{full_access:false, is_handler:false}` for a non-involved member; `ci_stats` and the roster `ci_export` are full-only; every export is audited `CI_EXPORTED`. | `20261103120000_confidential_informants.sql` |
| `public.ci_create` / `ci_update` / `ci_set_status` / `ci_handler_set` / `ci_handler_remove` / `ci_capacity_request_submit/_decide/_withdraw` / `ci_capacity_set` | Designation (self as primary, or CI command for any handler — `unavailable` is the one wording for an unusable person), edits, status (full only, reason, `compromised` fans out), handler changes (full only; access ends the moment the row ends; `primary_required`), and capacity — 6 by default, the `capacity` refusal for a handler / the named override wording for command, `p_override_reason` → `CI_CAPACITY_OVERRIDE` + a raised `limit_override`; requests → reviewers (`ci_reviewers`) → decide (full only). Authority refusals raise `P0403` (`private.perm_raise`); validation returns `{ok:false, code, message}`. | same |
| `public.ci_contact_log/_update/_delete` / `ci_assess` / `ci_intel_create/_update/_set_corroboration/_links_set/_delete` / `ci_case_link/_unlink` / `ci_payment_record/_approve` | The handler's work on their own sources: contacts (last / next contact), assessments (reliability / risk copied onto the CI), intelligence on a readable case (auto case link; every mention target must be visible — `bad_link`; the other handlers told with ids only), corroboration separate from reliability, payments (recordkeeping). Audit in `ci_audit_events` — **never `audit_log`**. | same |
| `public.ci_release(p_intel, p_title, p_body, p_handling)` / `ci_release_revoke(p_release, p_reason)` | Full only: `private.ci_sanitized` refuses a text naming the CI number / person / alias / handler (`unsanitized`); a clean release writes the visible `case_intel_releases` row (no CI column — readable through the case) and the restricted `ci_releases` link; notifies the case lead with a kind that names no CI; the intelligence row is untouched. | same |
| `public.ci_sweep_run()` / `rls_test_ci_sweep(p_ci)` | Owner re-run of `private.ci_sweep()` (cron `ci-contact-sweep`, :40 hourly — overdue and 30-day-silent sources → `ci_contact_overdue`, ids only) answering `{ok:false, code:'denied'}` otherwise; the fixture-scoped runner for the suites. | same |
