    ...
    function openPersonModal(record, prefill) {
      ...
      node.innerHTML = `
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div><label class="mb-1 block text-xs font-semibold text-slate-400">Active BOLO</label><select data-k="bolo" class="w-full rounded-lg border border-white/10 bg-ink-900 px-3 py-2 text-sm text-white outline-none focus:border-badge-500"><option value="false" ${!p.bolo ? 'selected' : ''}>No</option><option value="true" ${p.bolo ? 'selected' : ''}>Yes — be on the lookout</option></select></div>
      ...
      node.querySelector('#p-save').onclick = async () => {
        ...
        payload.ccw = payload.ccw === 'true'; payload.bolo = payload.bolo === 'true'; payload.vch = Number(payload.vch) || 0; payload.felony_count = Number(payload.felony_count) || 0;
