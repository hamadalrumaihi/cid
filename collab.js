...
"use strict";
    ...
    function visibleAnnouncements() {
      const me = DB() && DB().me; const div = me ? me.division : null;
      return ANNOUNCEMENTS.filter((a) => a.audience === 'all' || a.audience === div).sort((a, b) => (Number(b.pinned) - Number(a.pinned)) || (new Date(b.created_at) - new Date(a.created_at)));
    }
    ...
    function markAnnouncementsSeen() { const latest = visibleAnnouncements()[0]; if (latest && typeof Store !== 'undefined') Store.set('annSeen', latest.created_at); updateAnnounceBadge(); }
    function updateAnnounceBadge() {
      const badge = $('#ann-nav-badge'); if (!badge) return;
      const seen = (typeof Store !== 'undefined' && Store.get('annSeen', '')) || '';
      const unread = visibleAnnouncements().filter((a) => a.created_at > seen).length;
    ...
    function renderAnnouncements() {
      ...
      if (!items.length) { wrap.innerHTML = `<div class="rounded-2xl border border-white/5 bg-ink-900/60 p-10 text-center"><p class="text-3xl">📣</p><p class="mt-2 text-sm text-slate-400">No announcements yet.${canAnnounceClient() ? ' Use “+ New Announcement” to post the first.' : ''}</p></div>`; return; }
      const canManage = canAnnounceClient();
      wrap.innerHTML = items.map((a) => `
        <article class="rounded-2xl border ${a.pinned ? 'border-amber-500/30 bg-amber-500/[0.04]' : 'border-white/5 bg-ink-900/60'} p-5">
          <div class="flex items-start justify-between gap-3">
            ...
            ${canManage ? `<div class="flex flex-shrink-0 gap-2"><button class="ann-edit text-[11px] text-slate-400 hover:text-white" data-id="${a.id}">edit</button></div>` : ''}
          </div>
          <p class="mt-3 whitespace-pre-wrap text-sm leading-relaxed text-slate-300">${esc(a.body)}</p>
        </article>`).join('');
      wrap.querySelectorAll('.ann-edit').forEach((b) => b.onclick = () => openAnnouncementModal(ANNOUNCEMENTS.find((x) => x.id === b.dataset.id)));
    ...
    function openAnnouncementModal(record) {
      ...
      node.innerHTML = `
        ...
        <div class="space-y-3">
          ...
          <div><label class="mb-1 block text-xs font-semibold text-slate-400">Message *</label><textarea id="an-body" rows="6" class="w-full rounded-lg border border-white/10 bg-ink-900 px-3 py-2 text-sm text-white outline-none focus:border-badge-500">${esc(a.body || '')}</textarea></div>
      ...
      node.querySelector('#an-save').onclick = async () => {
        ...
        const payload = { title, body: bodyv, audience: node.querySelector('#an-aud').value, pinned: node.querySelector('#an-pin').checked, author_name: DB().me.display_name };
