...
"use strict";
    ...
    async function fetchBenches() { if (!dbReady()) { renderBenches(); return; } try { BENCHES_CACHE = await DB().list('ballistics_benches', { order: 'name', ascending: true }); renderBenches(); } catch (e) { $('#bench-list').innerHTML = '<p class="text-sm text-rose-300">Couldn’t load — ' + escapeHTML(e.message || String(e)) + '</p>'; } }
