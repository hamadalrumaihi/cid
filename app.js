...
"use strict";
    ...
    function packetDocx(c, d) {
      ...
      if (c.notes && c.notes.trim()) { P.push({ text: 'Case Notes', style: 'heading' }); c.notes.replace(/\r\n?/g, '\n').split('\n').forEach((ln) => P.push({ text: ln, style: 'normal' })); }
    ...
    function packetPdf(c, d) {
      ...
      if (c.notes && c.notes.trim()) { y += 4; line('Case Notes', 12, true); c.notes.replace(/\r\n?/g, '\n').split('\n').forEach((ln) => line(ln || ' ', 10, false)); }
    ...
    function packetXlsx(c, d) {
      ...
      if (c.notes && c.notes.trim()) X.utils.book_append_sheet(wb, X.utils.aoa_to_sheet(c.notes.replace(/\r\n?/g, '\n').split('\n').map((l) => [l])), 'Notes');
    ...
    // Generic text-file download (used for Markdown export; matches downloadCsv).
    function downloadTextFile(filename, text, mime) {
      const blob = new Blob([text], { type: (mime || 'text/plain') + ';charset=utf-8' });
      const a = document.createElement('a'); const url = URL.createObjectURL(blob);
      a.href = url; a.download = filename; document.body.appendChild(a); a.click(); a.remove(); setTimeout(() => URL.revokeObjectURL(url), 1000);
    }
    // Compile the whole case to a portable Markdown document (paste into Obsidian,
    // Discord, a doc — anywhere). Same content as the other packet formats.
    function caseToMarkdown(c, d) {
      const L = [];
      L.push('# Case Packet — ' + c.case_number); L.push('');
      L.push('`' + [c.title, c.bureau, String(c.status).toUpperCase()].filter(Boolean).join(' · ') + '` · prepared ' + new Date().toLocaleString('en-US')); L.push('');
      L.push('## Summary'); L.push(c.summary || '—'); L.push('');
      if (c.notes && c.notes.trim()) { L.push('## Notes'); L.push(c.notes.replace(/\r\n?/g, '\n')); L.push(''); }
      L.push('## Evidence (' + d.ev.length + ')');
      d.ev.length ? d.ev.forEach((e) => L.push('- ' + (e.item_code ? e.item_code + ' — ' : '') + (e.description || e.type || 'item') + ' [' + e.tamper + ']')) : L.push('_None._'); L.push('');
      L.push('## Reports (' + d.rep.length + ')');
      d.rep.length ? d.rep.forEach((r) => L.push('- ' + reportTitle(r) + (r.finalized ? ' (finalized)' : '') + ' — ' + new Date(r.created_at).toLocaleDateString('en-US'))) : L.push('_None._'); L.push('');
      L.push('## Charges (' + d.charges.length + ')');
      d.charges.length ? d.charges.forEach((x) => L.push('- ' + x.code + ' — ' + x.title + (x.count > 1 ? ' ×' + x.count : '') + (x.level ? ' [' + x.level + ']' : '') + (x.jail != null ? ' · ' + penalSentence(x.jail) : '') + (x.fine != null ? ' · ' + fmtUSD(x.fine) : ''))) : L.push('_None._'); L.push('');
      L.push('## Named persons (' + d.persons.length + ')');
      d.persons.length ? d.persons.forEach((p) => L.push('- ' + p.name + (p.alias ? ' "' + p.alias + '"' : '') + (p.status ? ' — ' + p.status : ''))) : L.push('_None linked._'); L.push('');
      L.push('## Media (' + d.media.length + ')');
      d.media.length ? d.media.forEach((m) => L.push('- ' + (m.title || m.type) + ' — ' + (m.external_url || m.storage_path || ''))) : L.push('_None._'); L.push('');
      L.push('## RICO'); L.push(d.rico[0] ? ('Enterprise linked; ' + d.preds.length + ' predicate act(s).') : 'No RICO tracker for this case.');
      L.push(''); L.push('---'); L.push('_Generated by the CID Portal._');
      return L.join('\n');
    }
    function packetMarkdown(c, d) { downloadTextFile(slug(c.case_number) + '-packet.md', caseToMarkdown(c, d), 'text/markdown'); return true; }
    function exportCasePacket(c) {
      ...
      node.innerHTML = `
        ...
        <div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
          ...
          <button data-fmt="md" class="pk-fmt rounded-lg border border-white/10 bg-white/5 px-3 py-4 text-sm font-semibold text-white transition hover:bg-white/10">📝<br>.md</button>
        </div>
      ...
      node.querySelectorAll('.pk-fmt').forEach((b) => b.onclick = async () => {
        ...
        if (fmt === 'pdf') await ensureJsPDF(); else if (fmt === 'xlsx') await ensureXLSX();
        let ok = true;
        if (fmt === 'docx') packetDocx(c, d); else if (fmt === 'pdf') ok = packetPdf(c, d); else if (fmt === 'xlsx') ok = packetXlsx(c, d); else ok = packetMarkdown(c, d);
    ...
    async function supaSearch(q) {
      ...
      // Remember recent searches (deduped, most-recent first, capped at 8).
      const recents = [q].concat((Store.get('recentSearches', []) || []).filter((x) => x.toLowerCase() !== q.toLowerCase())).slice(0, 8);
      Store.set('recentSearches', recents);
      const node = el('div', { class: 'p-6' });
    ...
    function init() {
      ...
      wireDrawer(); wireCollapse(); if (typeof wireAppearance === "function") wireAppearance(); wireAllImports();
      // Central command
      ...
      initInbox();
      // Feedback tab (owner-triage + member submit)
