...
"use strict";
    ...
    /* ---- Audit Log viewer (Director / Command) ---- */
    let AUDIT_LOG = [];
    async function fetchAuditLog() {
      if (!dbReady() || !(DB() && DB().isAdmin())) { renderAuditLog(); return; }
    ...
    function renderAuditLog() {
      ...
      wrap.innerHTML = `<div class="rounded-2xl border border-white/5 bg-ink-900/60 p-6">
        <div class="mb-3 flex flex-wrap items-center justify-between gap-2"><div><h4 class="text-sm font-semibold uppercase tracking-wider text-amber-300/80">🧾 Audit Log</h4><p class="text-xs text-slate-400">Every create/update/delete across the division (newest first).</p></div><input id="audit-search" value="${esc(q)}" placeholder="Filter action / entity / officer…" class="w-60 rounded-lg border border-white/10 bg-ink-900 px-3 py-1.5 text-xs text-white outline-none focus:border-badge-500" /></div>
