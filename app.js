(function () {
    ...
    const BENCHES = {
      street: [
        { name: 'Grove St. Zip-Gun Workshop', tier: 'Low', outputs: ['Zip-guns', 'Serial-number grinding', 'Modified .38 pistols'],
          components: ['Scrap steel tubing', 'Springs', 'Improvised firing pins', 'Electrical tape'], case: '[LSB] Case-1000007', heat: 'Active' },
        { name: 'Davis Pistol Conversion Bench', tier: 'Low', outputs: ['Auto-sear conversions', 'Switch-equipped Glocks', 'Filed serials'],
          components: ['Polymer pistol frames', 'Auto-sear blanks', 'Files', 'Hand drills'], case: '[LSB] Case-1000001', heat: 'Surveillance' },
      ],
      organized: [
        { name: 'Cartel CNC Rifle Foundry', tier: 'High', outputs: ['Class 3 military rifles', 'Suppressors', 'Custom attachments'],
          components: ['Steel receivers', 'CNC billet blanks', 'Firing pins', 'Threaded barrels', 'Optics'], case: '[SAB] Case-9000001', heat: 'Active' },
        { name: 'Paleto Syndicate SMG Line', tier: 'High', outputs: ['Class 2 submachine guns', 'Full-auto kits', 'Box magazines'],
          components: ['Polymer frames', 'CNC machinery', 'Recoil springs', 'Bolt carriers'], case: '[BCB] Case-2000001', heat: 'Raid Pending' },
      ],
    };
    const BALLISTIC_LOG = [
      { sig: 'BLSTC-77-A · 9mm striations', weapon: 'Switch-modified Glock', gang: 'Davis Ballas', case: '[LSB] Case-1000007' },
      { sig: 'BLSTC-12-C · 5.56 chamber mark', weapon: 'CNC Class 3 rifle', gang: 'Vagos Cartel cell', case: '[SAB] Case-9000001' },
      { sig: 'BLSTC-49-B · filed serial #PX-0', weapon: 'Auto-sear pistol', gang: 'Grove St. set', case: '[LSB] Case-1000001' },
    ];
    ...
    function renderBenches() {
      renderBenchTabs();
      const wrap = $('#bench-list'); wrap.innerHTML = '';
      BENCHES[benchType].forEach((b) => {
        const tierTint = b.tier === 'High' ? 'border-rose-500/30 bg-rose-500/5 text-rose-300' : 'border-amber-500/30 bg-amber-500/5 text-amber-300';
        ...
        wrap.appendChild(el('div', { class:'rounded-2xl border border-white/5 bg-ink-900/60 p-6' }, `
          <div class="flex flex-wrap items-start justify-between gap-3">
            <div><h4 class="text-base font-semibold text-white">${esc(b.name)}</h4><p class="mt-1 text-xs text-slate-400">Linked investigation: <span class="font-mono text-blue-300">${esc(b.case)}</span></p></div>
            <div class="flex items-center gap-2"><span class="rounded-md border px-2.5 py-1 text-[10px] font-semibold uppercase ${tierTint}">${b.tier}-Tier</span><span class="rounded-md px-2.5 py-1 text-[10px] font-semibold uppercase ${heatTint}">${esc(b.heat)}</span></div>
          </div>
          <div class="mt-4 grid grid-cols-1 gap-5 sm:grid-cols-2">
            <div><p class="mb-2 text-xs font-semibold uppercase tracking-wider text-blue-300/70">Manufacturing Outputs</p><div class="flex flex-wrap gap-2">${b.outputs.map((o)=>`<span class="rounded-full border border-white/10 bg-ink-850 px-3 py-1 text-xs text-slate-200">${esc(o)}</span>`).join('')}</div></div>
            <div><p class="mb-2 text-xs font-semibold uppercase tracking-wider text-blue-300/70">Component Tracing</p><div class="space-y-1.5">${b.components.map((c)=>`<div class="flex items-center gap-2 text-xs text-slate-300"><span class="h-1.5 w-1.5 rounded-full bg-blue-400"></span>${esc(c)}</div>`).join('')}</div></div>
          </div>`));
      });
    }
    function renderBallisticLog() {
      const wrap = $('#ballistic-log'); wrap.innerHTML = '';
      BALLISTIC_LOG.forEach((l) => wrap.appendChild(el('div', { class:'rounded-xl border border-white/10 bg-ink-900 p-3' },
        `<p class="font-mono text-xs text-violet-300">${esc(l.sig)}</p><p class="mt-1 text-sm text-white">${esc(l.weapon)}</p><div class="mt-1.5 flex items-center justify-between text-[11px]"><span class="text-slate-400">${esc(l.gang)}</span><span class="font-mono text-blue-300">${esc(l.case)}</span></div>`)));
    }
...
Plainclothes is standard. Tactical loadouts require Bureau Lead approval.` },
    ...
    let PLACES = Store.get('places', null) || [
      ...
      { id:'p2', name:'Forum Dr. Stash', type:'Stash House', area:'Davis', gangId:'g1', case:'[LSB] Case-1000007', drug:'' },
    ];
    const savePlaces = () => Store.set('places', PLACES);

    let REPORTS = Store.get('reports', null) || {};
    const saveReports = () => Store.set('reports', REPORTS);
    ...
    function autoVal(key, caseId) {
      ...
      if (key === 'bureau') return bureauOf(caseId);
      if (key === 'detective' || key === 'affiant') return 'Det. Oliver Och · 915 · MCB';
    ...
    function productionRecipe(drugName) {
      const d = DRUGS.find((x) => x.name === drugName); if (!d) return [];
      return [ `Acquire precursors: ${d.precursors.map((p)=>p.n).join(', ')}`, `Synthesize / cook ${d.name} base`, `Cut to street purity grade`, `Package into distribution units`, `Distribute to hotspot: ${d.hotspots[0] ? d.hotspots[0].area : 'TBD'}` ];
    }
    function renderPlaces() {
      const grid = $('#place-grid'); grid.innerHTML = '';
      if (!PLACES.length) { grid.innerHTML = '<p class="text-sm text-slate-500">No locations logged.</p>'; return; }
      PLACES.forEach((p) => {
        const gang = GANGS.find((g) => g.id === p.gangId);
        const recipe = p.type === 'Drug Lab' && p.drug ? productionRecipe(p.drug) : [];
        const card = el('div', { class:'rounded-2xl border border-white/5 bg-ink-900/60 p-6' });
        card.innerHTML = `
          <div class="flex items-start justify-between gap-3">
            <div><h4 class="text-base font-semibold text-white">${esc(p.name)}</h4><p class="mt-0.5 text-xs text-slate-400">${esc(p.type)} · ${esc(p.area||'—')}</p></div>
            <div class="flex items-center gap-2"><button class="pl-edit rounded-md border border-white/10 bg-white/5 px-2.5 py-1 text-xs text-slate-200 transition hover:bg-white/10">Edit</button><button class="pl-del rounded-md border border-white/10 bg-white/5 px-2.5 py-1 text-xs text-rose-300 transition hover:bg-rose-500/10">✕</button></div>
          </div>
          <div class="mt-3 flex flex-wrap gap-2 text-[11px]">
            ${gang?`<span class="rounded-md bg-violet-500/10 px-2 py-1 text-violet-300">🚩 ${esc(gang.name)}</span>`:''}
            ${p.case?`<span class="rounded-md bg-blue-500/10 px-2 py-1 font-mono text-blue-300">${esc(p.case)}</span>`:''}
            ${p.drug?`<span class="rounded-md bg-emerald-500/10 px-2 py-1 text-emerald-300">💊 ${esc(p.drug)}</span>`:''}
          </div>
          ${recipe.length ? `<div class="mt-4"><p class="mb-2 text-[10px] font-semibold uppercase tracking-wider text-blue-300/70">Production Process</p><div class="space-y-1.5">${recipe.map((s,i)=>`<div class="flex items-center gap-2 text-xs text-slate-300"><span class="grid h-5 w-5 flex-shrink-0 place-items-center rounded-full bg-blue-500/15 font-mono text-[10px] text-blue-300">${i+1}</span>${esc(s)}${i<recipe.length-1?'<span class="text-slate-600">→</span>':''}</div>`).join('')}</div></div>` : ''}`;
        card.querySelector('.pl-edit').addEventListener('click', () => openPlaceModal(p));
        card.querySelector('.pl-del').addEventListener('click', () => { if (confirm(`Delete location "${p.name}"?`)) { PLACES = PLACES.filter((x)=>x.id!==p.id); savePlaces(); renderPlaces(); toast('Location deleted', 'warn'); } });
        grid.appendChild(card);
      });
    }
    function openPlaceModal(existing) {
      const p = existing ? { ...existing } : { id: uid('p'), name:'', type:'Drug Lab', area:'', gangId:'', case:'', drug:'' };
      const node = el('div', { class:'p-6' });
      node.innerHTML = `
        <div class="mb-5 flex items-center justify-between"><h3 class="text-xl font-bold text-white">${existing?'Edit':'New'} Location</h3><button class="close-x text-slate-400 hover:text-white text-2xl leading-none">&times;</button></div>
        <div class="space-y-3">
          <div><label class="mb-1 block text-xs font-semibold text-slate-400">Name *</label><input id="pl-name" class="w-full rounded-lg border border-white/10 bg-ink-900 px-3 py-2 text-sm text-white outline-none focus:border-badge-500" value="${esc(p.name)}" /></div>
          <div class="grid grid-cols-2 gap-3">
            <div><label class="mb-1 block text-xs font-semibold text-slate-400">Type</label><select id="pl-type" class="w-full rounded-lg border border-white/10 bg-ink-900 px-3 py-2 text-sm text-white outline-none focus:border-badge-500">${PLACE_TYPES.map((t)=>`<option ${t===p.type?'selected':''}>${t}</option>`).join('')}</select></div>
            <div><label class="mb-1 block text-xs font-semibold text-slate-400">Area</label><input id="pl-area" class="w-full rounded-lg border border-white/10 bg-ink-900 px-3 py-2 text-sm text-white outline-none focus:border-badge-500" value="${esc(p.area)}" /></div>
          </div>
          <div class="grid grid-cols-2 gap-3">
            <div><label class="mb-1 block text-xs font-semibold text-slate-400">Controlling Gang</label><select id="pl-gang" class="w-full rounded-lg border border-white/10 bg-ink-900 px-3 py-2 text-sm text-white outline-none focus:border-badge-500"><option value="">— none —</option>${GANGS.map((g)=>`<option value="${g.id}" ${g.id===p.gangId?'selected':''}>${esc(g.name)}</option>`).join('')}</select></div>
            <div><label class="mb-1 block text-xs font-semibold text-slate-400">Linked Case</label><select id="pl-case" class="w-full rounded-lg border border-white/10 bg-ink-900 px-3 py-2 text-sm text-white outline-none focus:border-badge-500"><option value="">— none —</option>${ACTIVE_CASES.map((c)=>`<option ${c===p.case?'selected':''}>${esc(c)}</option>`).join('')}</select></div>
          </div>
          <div id="pl-drug-wrap"><label class="mb-1 block text-xs font-semibold text-slate-400">Produced Narcotic (labs only)</label><select id="pl-drug" class="w-full rounded-lg border border-white/10 bg-ink-900 px-3 py-2 text-sm text-white outline-none focus:border-badge-500"><option value="">— none —</option>${DRUGS.map((d)=>`<option ${d.name===p.drug?'selected':''}>${esc(d.name)}</option>`).join('')}</select></div>
        </div>
        <button id="pl-save" class="mt-5 w-full rounded-lg bg-gradient-to-r from-badge-500 to-blue-700 py-3 text-sm font-semibold text-white shadow-glow transition hover:brightness-110">Save Location</button>`;
      node.querySelector('.close-x').onclick = closeModal;
      const typeSel = node.querySelector('#pl-type'), drugWrap = node.querySelector('#pl-drug-wrap');
      const syncDrug = () => drugWrap.style.display = typeSel.value === 'Drug Lab' ? '' : 'none';
      syncDrug(); typeSel.onchange = syncDrug;
      node.querySelector('#pl-save').onclick = () => {
        const name = node.querySelector('#pl-name').value.trim();
        if (!name) { toast('Location name is required.', 'warn'); return; }
        Object.assign(p, { name, type:typeSel.value, area:node.querySelector('#pl-area').value.trim(), gangId:node.querySelector('#pl-gang').value, case:node.querySelector('#pl-case').value, drug: typeSel.value==='Drug Lab' ? node.querySelector('#pl-drug').value : '' });
        if (existing) { const i = PLACES.findIndex((x)=>x.id===p.id); PLACES[i] = p; } else PLACES.push(p);
        savePlaces(); renderPlaces(); closeModal(); toast(`Location "${p.name}" saved`, 'success');
      };
      openModal(node);
    }
    ...
    function renderTemplateList() {
      const wrap = $('#template-list'); wrap.innerHTML = '';
      REPORT_TEMPLATES.forEach((t) => {
        ...
        b.addEventListener('click', () => openReportModal(t.id, $('#report-case').value, null, 'initial'));
        wrap.appendChild(b);
      });
    }
    function fillCaseSelect(sel) { sel.innerHTML = ACTIVE_CASES.map((c)=>`<option>${esc(c)}</option>`).join(''); }
    ...
    function renderReportChain() {
      const caseId = $('#report-case').value;
      const list = (REPORTS[caseId] || []).slice().sort((a,b) => a.created - b.created);
      $('#chain-count').textContent = `${list.length} report${list.length===1?'':'s'}`;
      const wrap = $('#report-chain'); wrap.innerHTML = '';
      if (!list.length) { wrap.innerHTML = '<p class="text-sm text-slate-500">No reports for this case yet. Pick a template to generate the initial report.</p>'; return; }
      list.forEach((r) => {
        const tpl = tplById(r.templateId);
        const card = el('div', { class:'rounded-xl border border-white/10 bg-ink-900 p-4' });
        card.innerHTML = `
          <div class="flex flex-wrap items-center justify-between gap-2">
            <div class="flex items-center gap-2">${reportKindBadge(r)}<span class="text-sm font-semibold text-white">${esc(tpl ? tpl.name : 'Report')}</span></div>
            <span class="text-[11px] text-slate-500">${esc(r.author)} · ${new Date(r.created).toLocaleDateString('en-US')}</span>
          </div>
          ${r.parentId?`<p class="mt-1 text-[11px] text-slate-500">↳ linked to prior report</p>`:''}
          <div class="mt-3 flex flex-wrap gap-2">
            ...
            <button class="r-docx rounded-md border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-slate-200 transition hover:bg-white/10">Export .docx</button>
            <button class="r-supp rounded-md border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-violet-300 transition hover:bg-white/10">+ Supplemental</button>
            <button class="r-follow rounded-md border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-amber-300 transition hover:bg-white/10">+ Follow-up</button>
          </div>`;
        card.querySelector('.r-view').onclick = () => viewReport(r);
        card.querySelector('.r-docx').onclick = () => exportReportDocx(r);
        card.querySelector('.r-supp').onclick = () => openReportModal(r.templateId, caseId, r.id, 'supplemental');
        card.querySelector('.r-follow').onclick = () => openReportModal(r.templateId, caseId, r.id, 'followup');
        wrap.appendChild(card);
      });
    }
    function openReportModal(templateId, caseId, parentId, kind) {
      const tpl = tplById(templateId); if (!tpl) return;
      const node = el('div', { class:'p-6' });
      const seq = kind === 'initial' ? 0 : (REPORTS[caseId] || []).filter((r) => r.kind === kind).length + 1;
      ...
      node.innerHTML = `
        ...
        ${parentId?`<p class="mb-4 rounded-lg border border-white/10 bg-ink-900 p-2.5 text-xs text-slate-400">↳ Linked as ${kind} to a prior report on <span class="font-mono text-blue-300">${esc(caseId)}</span>.</p>`:''}
        <div class="space-y-3">
          ${tpl.sections.map((s) => {
      ...
      node.querySelector('#r-save').onclick = () => {
        ...
        const rec = { id: uid('r'), caseId, templateId, kind, seq, parentId: parentId || null, author: 'Det. Oliver Och (915)', created: Date.now(), fields };
        (REPORTS[caseId] = REPORTS[caseId] || []).push(rec); saveReports();
        renderReportChain(); closeModal(); toast(`${heading} saved to ${caseId}`, 'success');
      };
      openModal(node);
    }
    function viewReport(r) {
      ...
      node.innerHTML = `
        ...
        <div class="rounded-xl border border-white/10 bg-ink-900 p-5">
          ...
          <h2 class="mt-1 text-center text-xl font-bold text-white">${esc(tpl?tpl.name:'Report')} ${r.kind!=='initial'?'· '+(r.kind==='supplemental'?'Supplemental #'+r.seq:'Follow-up #'+r.seq):''}</h2>
          <p class="mt-1 text-center text-xs text-slate-400">${esc(r.caseId)} · ${esc(r.author)} · ${new Date(r.created).toLocaleString('en-US')}</p>
          <div class="mt-5 space-y-3">${tpl.sections.map((s)=>{const v=r.fields[s.key]||'—'; return `<div><p class="text-[11px] font-semibold uppercase tracking-wider text-slate-400">${esc(s.label)}</p><p class="mt-0.5 whitespace-pre-line text-sm text-slate-200">${esc(v)}</p></div>`;}).join('')}</div>
        </div>
        <div class="mt-5 flex flex-wrap gap-3 no-print"><button onclick="window.print()" class="rounded-lg bg-gradient-to-r from-badge-500 to-blue-700 px-5 py-2.5 text-sm font-semibold text-white shadow-glow transition hover:brightness-110">🖨️ Print</button><button id="v-docx" class="rounded-lg border border-white/10 bg-white/5 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-white/10">Export .docx</button></div>`;
      node.querySelector('.close-x').onclick = closeModal;
    ...
    function exportReportDocx(r) {
      const tpl = tplById(r.templateId);
      const title = `${tpl?tpl.name:'Report'}${r.kind!=='initial'?' — '+(r.kind==='supplemental'?'Supplemental #'+r.seq:'Follow-up #'+r.seq):''}`;
      const paras = [ {text:'Criminal Investigation Division — State of San Andreas', style:'subtitle'}, {text:title, style:'title'}, {text:`${r.caseId}  ·  ${r.author}  ·  ${new Date(r.created).toLocaleString('en-US')}`, style:'subtitle'}, {text:'', style:'normal'} ];
      tpl.sections.forEach((s) => { paras.push({text:s.label, style:'heading'}); paras.push({text: r.fields[s.key] || '—', style:'normal'}); });
      downloadDocx(title, paras, `${r.caseId.replace(/[^a-z0-9]/gi,'-')}-${r.kind}-${r.seq||0}.docx`);
